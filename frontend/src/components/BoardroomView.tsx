...
export default function BoardroomView() {
  ...
  useEffect(() => {
    ...
    const tabMap = ['ceo', 'ceo', 'strategy', 'marketing', 'sales', 'finance'];
    if (currentRunningIndex >= 0 && currentRunningIndex < tabMap.length) {
      setActiveTab(tabMap[currentRunningIndex]);
    }
    ...
    <div className="p-8 max-w-6xl mx-auto space-y-8 overflow-y-auto max-h-screen custom-scrollbar pb-16">
      ...
      <div className="bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-neutral-100 pb-4">
          <div>
            <h2 className="text-sm font-bold text-neutral-800 uppercase tracking-wide">Agent Output Inspector</h2>
            <p className="text-[11px] text-neutral-400 mt-0.5">Explore detailed briefs and blueprints generated by each boardroom unit.</p>
          </div>
          
          {/* Tabs */}
          <div className="flex flex-wrap gap-1 bg-neutral-100 p-1 rounded-xl border border-neutral-200/40 shrink-0">
            {[
              { id: 'ceo', label: '👔 CEO' },
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all cursor-pointer uppercase tracking-wider ${
                  activeTab === tab.id
                    ? 'bg-white text-neutral-800 shadow-sm border border-neutral-200/60'
